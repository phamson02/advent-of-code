...
fn solution(input: &str) -> String {
    let n_bytes = input.len() as i32;

    let n = (1..n_bytes)
        .find(|&n_corrupted| {
            let puzzle = Puzzle::new(&input, 71, n_corrupted);
            let start = puzzle.start;
            let end = puzzle.end;

            dijkstra(&start, |pos| pos.successors(&puzzle), |pos| *pos == end).is_none()
        })
        .unwrap();

    input.lines().nth((n - 1) as usize).unwrap().to_string()
}
#[cfg(test)]
mod tests {
    use crate::solution;
